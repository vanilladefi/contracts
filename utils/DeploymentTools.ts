  ...
  query: gql`
    query balancer($blockNumber: Int, $weth: [String], $numHolders: Int) {
      pools(
        ...
        where: {
          tokensList_contains: $weth
          publicSwap: true
          holdersCount_gte: $numHolders
        }
        orderBy: liquidity
        orderDirection: desc
      ) {
...
export type ExchangeReserveEstimate = {
  token: Token
  balReserve: number
  sushiReserve: number
  uniswapReserve: number
}
type ExchangeReserveEstimates = Record<string, ExchangeReserveEstimate>
const getBalancerReserveEstimates = (
  poolTokens: BalancerPoolToken[],
): ExchangeReserveEstimates => {
  let wethReserve = 0
  let tokenWeights = 0
  ...
  for (const nonWethToken of nonWethTokens) {
    ...
    reserveEstimates[nonWethToken.address] = {
      token: nonWethToken,
      balReserve: estimate,
      sushiReserve: 0,
      uniswapReserve: 0,
...
export const queryCrossExchangeLiquidity = async (
  blockNumber: number,
  wethAddress: string,
  safeListCriteria: (t: ExchangeReserveEstimate) => boolean,
): Promise<ExchangeReserveEstimate[]> => {
  ...
  let balancerPools = await request<BalancerPools>(
    balancer.API,
    balancer.query,
    balancerVariables,
  )
  ...
  let sushiPairs = await request<UniswapLikePairs>(
    sushi.API,
    sushi.query,
    uniSushiVariables,
  )
  for (const { token, wethReserve } of [
    ...sushiPairs.token0,
    ...sushiPairs.token1,
  ]) {
    ...
    if (totalReserves[key]) {
      ...
      totalReserves[key] = {
        token,
        sushiReserve: Number(wethReserve),
        balReserve: 0,
        uniswapReserve: 0,
      }
    }
  ...
  let uniswapPairs = await request<UniswapLikePairs>(
    uniswapV2.API,
    uniswapV2.query,
    uniSushiVariables,
  )
  for (const { token, wethReserve } of [
    ...
    if (totalReserves[key]) {
      ...
      totalReserves[key] = {
        ...
        sushiReserve: 0,
        balReserve: 0,
        uniswapReserve: Number(wethReserve),
      }
    }
  }
...
export const connectUsing = (provider: Provider) => ({
  router: (address: string) =>
    VanillaV1Router02__factory.connect(address, provider),
  safelist: (address: string) =>
    VanillaV1Safelist01__factory.connect(address, provider),
  vnlToken: (address: string) =>
    VanillaV1Token02__factory.connect(address, provider),
  migrationState: (address: string) =>
    VanillaV1MigrationState__factory.connect(address, provider),
})
